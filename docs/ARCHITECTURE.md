...
个人档案、派生进度、无题文作答证据和最小会话状态以一个 IndexedDB 事务原子提交。作答行为模块用单调时钟累计前台有效用时、首次选择和有界改选次数；先按题干/选项阅读负荷和逻辑复杂度给出 12–90 秒总体参考，至少积累 6 条合格样本后才用个人 pace 聚合做 0.65–1.3 倍校正，个人参考下限为 8 秒。持久化 pace 始终相对不变的题目负荷分桶，避免个人校准反向污染自己的基线。多选构造不算改选，失焦/隐藏/刷新后的残缺计时不作熟练判断；基线未成熟或相对个人基线仍异常快的单题正确只说明信号不足，连续稳定证据成熟后才可能呈现熟练倾向；慢答但没有反复改选只标记审慎阅读。Web 不再要求考生自报把握度：只有答对、连续前台计时完整、相对当前基线处于正常或审慎区间且没有真实改选，才会形成稳定正确证据；快答正确、计时不完整或改选会安排复测，所有答错都会进入稳定准确率分母。行为信号不改写固定答案键的客观对错，只决定正确作答能否成为稳定掌握证据。刷新时只读恢复档案和派生进度，不持久化 active question、原始 response、题干、选项、答案或解析。案例与论文在 Web MVP 中保持未测量。
