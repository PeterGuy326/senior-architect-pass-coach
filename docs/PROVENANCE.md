...
- 作答前只抽取当前一道题的公开题面；提交后参考答案和解析会作为已展示反馈写入 owner-only 私人会话，显式合格 `agent-host` 模式也会把它们交给所选 Host，但它们不会写入本仓库或员工包；Codex 个人实验模式不接收题干、选项、作答、参考答案或解析，只接收科目、考点、掌握结果与去身份化进度；
