...
答题用时只是保守的辅助信号，不是能力诊断或身份特征。页面按题干/选项阅读负荷估算参考时间；至少 6 条合格记录后，才使用最多 48 条权重的离散 pace 分桶形成个人节奏基线，不保存逐次选择轨迹。页面只累计题目可答且处于前台焦点时的时间；隐藏、失焦、打开 Agent 面板或刷新会让本题计时中断，中断数据不用于推断熟练度。行为信号只能降低证据等级或提前复测，绝不会把用户自报的 `unsure/guess` 单题升级为 `mastered`；慢读且未反复改选也不会单独判为不会。
...
用户显式连接 Local Agent Runtime 并选择兼容引擎时，本轮去标识化进度摘要、自然语言追问和必要的公开题面会交给所选 Agent Host 及其配置的模型服务；提交后，本地判题产生的作答、参考答案和解析也会作为受信材料进入该轮。去标识化摘要可以携带 `revision_heavy`、`fast_wrong_guess_risk`、`clean_confident_correct` 等有界派生原因码，但不会携带精确答题秒数、焦点记录、改选次数、个人节奏基线或选择轨迹。是否离开本机取决于该 Host 的模型服务和账号配置。系统不会静默从 `content-only` 切换到模型模式，Agent 失败也不会改写或回滚浏览器进度。
