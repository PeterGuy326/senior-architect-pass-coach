...
答题用时是自动判断证据强度的主要行为信号，但不是对人的能力诊断或身份特征。页面按题干/选项阅读负荷估算参考时间；至少 12 条干净记录后，才使用最多 48 条权重的离散 pace 分桶形成个人节奏基线，不保存逐次选择轨迹。页面只累计题目可答且处于前台焦点时的时间；隐藏、失焦、打开 Agent 面板或刷新会让本题计时中断，中断数据不用于推断熟练度。考生无需自报把握度；系统只把“答对、连续计时完整、相对当前基线处于正常区间、首次选择不过早且未真实改选”的作答作为稳定正确证据。过快、过慢、计时不完整或真实改选都会提前复测；固定答案键仍独立决定客观对错。
