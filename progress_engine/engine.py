...
The AI coach owns the conversation; this package owns durable evidence. Learner
data defaults to the operating system's per-user application-data directory,
while public study material is supplied separately with ``--content-dir``.
The engine never performs network requests.
"""
...
CURRICULUM_PATH = PACKAGE_ROOT / "config" / "curriculum.json"
APP_DIRECTORY_NAME = "senior-architect-pass-coach"
...
def default_data_dir() -> Path:
    """Return an OS-appropriate private, per-user application-data directory."""

    override = os.environ.get("SENIOR_ARCHITECT_DATA_DIR")
    if override:
        return Path(override).expanduser()
    if os.name == "nt":
        base = os.environ.get("LOCALAPPDATA")
        return (Path(base) if base else Path.home() / "AppData" / "Local") / APP_DIRECTORY_NAME
...
def default_content_dir() -> Path:
    """Return the configured public-content clone root, without fetching it."""

    override = os.environ.get("SENIOR_ARCHITECT_CONTENT_DIR")
    return Path(override).expanduser() if override else PACKAGE_ROOT
...
def load_curriculum() -> dict[str, Any]:
    ...
    if not isinstance(curriculum, dict) or not isinstance(curriculum.get("topics"), list):
        raise TutorError("config/curriculum.json 缺少 topics 数组")
    ...
    for topic in curriculum["topics"]:
        ...
        if not isinstance(resources, list) or not resources:
            raise TutorError(f"课程表考点 {topic_id} 缺少资源映射")
        for resource in resources:
            if not isinstance(resource, str) or not resource.strip():
                raise TutorError(f"课程表考点 {topic_id} 包含无效资源映射")
            local_path = resource.split("#", 1)[0]
            parsed_path = Path(local_path)
            if parsed_path.is_absolute() or ".." in parsed_path.parts:
                raise TutorError(f"课程表考点 {topic_id} 的资源路径越界：{resource}")
        seen.add(topic_id)
...
def privacy_check(data_dir: Path) -> tuple[bool, str]:
    """Reject private state nested in Git unless the candidate is ignored.

    The default user-level directory naturally sits outside this source tree.
    For ``--data-dir`` overrides we discover the containing Git worktree, so a
    copied CLI cannot accidentally persist learner records in another clone.
    """
    ...
    if discovered.returncode != 0:
        return True, "私人目录位于 Git 仓库外"
    git_root = Path(discovered.stdout.strip()).resolve()
    try:
        relative_data_dir = resolved.relative_to(git_root)
    except ValueError:
        return True, "私人目录位于 Git 仓库外"
...
def doctor_checks(
    ...
    try:
        ...
        for topic in curriculum["topics"]:
            for resource in topic.get("resources", []):
                if not (content_dir / resource.split("#", 1)[0]).exists():
...
def cmd_doctor(args: argparse.Namespace) -> int:
    healthy, checks = doctor_checks(args.data_dir, args.content_dir)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="私人状态目录（默认：操作系统用户级应用数据目录）",
    )
    parser.add_argument(
        "--content-dir",
        type=Path,
        default=default_content_dir(),
        help="公开复习资料 clone 根目录（也可设置 SENIOR_ARCHITECT_CONTENT_DIR）",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    args.content_dir = args.content_dir.expanduser().resolve()
